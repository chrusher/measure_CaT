...
def redisperse(inputwavelengths, inputfluxes, firstWavelength=None, lastWavelength=None, dispersion=None, nPixels=None, outside=None, function='spline', ratio=False):
    ...
    if dispersion is None and nPixels is not None:

        if firstWavelength is None:
            firstWavelength = inputwavelengths[0]
        if lastWavelength is None:
            lastWavelength = inputwavelengths[-1]    
        outputwavelengths = np.linspace(firstWavelength, lastWavelength, nPixels)

    elif dispersion is not None and nPixels is None:
        if firstWavelength is None:
            firstWavelength = inputwavelengths[0]
        if lastWavelength is None:
            lastWavelength = inputwavelengths[-1] 
    ...
    elif dispersion is not None and nPixels is not None:
        if firstWavelength is not None:
            outputwavelengths = firstWavelength + dispersion * np.ones(nPixels)
        elif lastWavelength is not None:
    ...
    outputfluxes = interp(inputwavelengths, inputfluxes, inputedges, inputdispersions, outputwavelengths, outputedges, outside, function, ratio)
...
def interp(inputwavelengths, inputfluxes, inputedges, inputdispersions, outputwavelengths, outputedges, outside=None, function='spline', ratio=False):
    ...
    if function == 'nearest':
        ...
        for newpixel in range(outputfluxes.size):
            ...
            elif outputwavelengths[newpixel] > inputwavelengths[-1] and lastnew is None:
        ...
        if function == 'linear':
            fluxspline = interpolate.UnivariateSpline(inputwavelengths, fluxdensities, s=0, k=1)
        else:
...
def lineartolog(inputwavelengths, inputfluxes, outside=0, function='spline', ratio=False, logDispersion=0, nPixels=None):
    ...
    if logDispersion:
        ...
        if nPixels is None:
            nPixels = inputwavelengths.size

        outputedges = np.logspace(np.log10(inputedges[0]), np.log10(inputedges[-1]), nPixels + 1)    
...
def log_redisperse(inputwavelengths, inputfluxes, firstWavelength, lastWavelength, dispersion, outside=0, function='spline', ratio=False):
    ...
    if firstWavelength is None:
        firstWavelenght = inputwavelengths[0]
    if lastWavelength is None:
        lastWavelenght = inputwavelengths[-1]        
