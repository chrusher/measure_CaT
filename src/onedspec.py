...
def write_errors(name, wavelengths, fluxes, sigmas, clobber=False):
    ...
    fhdu.header.set('CRVAL1', wavelengths[0])
    fhdu.header.set('CDELT1', wavelengths[1] - wavelengths[0])
    fhdu.header.set('DC-FLAG', 0)

    shdu = fits.ImageHDU(sigmas)
    shdu.header.set('CRPIX1', 1)
    shdu.header.set('CRVAL1', wavelengths[0])
    shdu.header.set('CDELT1', wavelengths[1] - wavelengths[0])
    shdu.header.set('DC-FLAG', 0)

    hdulist = fits.HDUList([fhdu, shdu])
...
def writespectrum(name, wavelengths, fluxes, clobber=False):
    ...
    write(name, wavelengths, fluxes, clobber=False) 
